...
<img src="https://grostore.themetags.com/public/frontend/default/assets/img/shapes/leaf-gray.png?v=v4.4.0" alt="tomato"
    ...
    <div class="container">
        ...
        <h2 class="mb-2 text-center">Panier</h2>
        <nav>
            <ol class="breadcrumb justify-content-center">
                <li class="breadcrumb-item fw-bold" aria-current="page">
                    <a href="{{url('/')}}">Home</a>
                </li>
                <li class="breadcrumb-item fw-bold" aria-current="page">Carts</li>
            </ol>
        </nav>
    </div>
            </div>
    ...
    <section class="cart-section ptb-120">
        <div class="container">
            <div class="rounded-2 overflow-hidden">
                <table class="cart-table w-100 bg-white" id="tableCarte">
                    <thead>
                        <th>Image</th>
                ...
                <div class="col-xl-5">
                    <div class="cart-summery bg-white rounded-2 pt-4 pb-7 px-5 mt-4">
                        <table class="w-100">
                            <tr>
                                <td class="py-3">
                                    <h5 class="mb-0 fw-medium">Total</h5>
                                </td>
                                <td class="py-3">
                            ...
                            <tr class="coupon-discount-wrapper d-none">
                                <td class="py-3">
                        ...
                        <p class="mb-5 mt-2">Les options d'expédition seront mises à jour lors du paiement.</p>
                        <div class="btns-group d-flex flex-wrap gap-3">

                            <a href="{{url('/')}}"
                                class="btn btn-outline-secondary border-secondary btn-md rounded-1">Continuer les achats</a>

                            <a href="{{url('checkout')}}" type="submit"
                                class="btn btn-primary btn-md rounded-1">Caisse</a>
                        </div>
                    </div>
                </div>
...
<script>
            ...
            $.ajax({
                ...
                success: function (response)
                {
                    if(response.status==200)
                    {
                        ...
                        $('#tableCarte tbody').html("");
                        $.each(response.data, function (index, value)
                        {
                            var Path= "{{asset('storage/')}}" +"/"+ value.image;
                            $('#tableCarte tbody').append(`<tr>
                                <td class="h-100px">
                                    <img src="${Path}"
                                ...
                                <td>
                                    <span class="text-dark fw-bold me-2 d-lg-none">Prix ​​unitaire:</span>
                                ...
                                <td>
                                    <div class="product-qty d-inline-flex align-items-center">
                                        <button class="decrese btnminus" value=${value.id}>-</button>
                                        <input type="text" readonly value="${value.qte}">
                                        <button class="increase" value=${value.id}>+</button>
                                    </div>
                                </td>

                                <td>
                                    <span class="text-dark fw-bold me-2 d-lg-none">Prix ​​total:</span>
                                    <span class="text-dark fw-bold">
                                        ${value.total} DH
                                    </span>
                                </td>
                                <td>
                                    <span class="text-dark fw-bold me-2 d-lg-none">Supprimer:</span>
                                    <span class="text-dark fw-bold"><button type="button" class="close-btn ms-3"
                                            onclick="handleCartItem('delete', 2108)"><i
                                                class="fas fa-close"></i></button></span>
                                </td>
                            </tr>`);
                        });
                        $('.sub-total-price').text(response.Total +" DH");
                    }
                }
            });
    ...
    listCartHeader();
        ...
        $(document).on('click','.product-qty .btnminus',function(e)
        {
            e.preventDefault();
            ...
            var qteOld       =$(this).closest('tr').find('input[type="text"]').val()  ;
            var qtenew       = parseInt(qteOld) ;
            $.ajax({
                type: "get",
                url: "{{url('changeCart')}}",
                data:
        ...
        $(document).on('click','.product-qty .increase',function(e)
        {
            e.preventDefault();
            var quantityInput = $(this).siblings('input[type="text"]');
            var currentQuantity = parseInt(quantityInput.val());
